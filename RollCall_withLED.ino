 ...
 * Tracks s: ESP32, MFRC522 RFID Reader
 */
...
void loop() {
    ...
    if (!cardPresent && rfidReader.PICC_IsNewCardPresent() && rfidReader.PICC_ReadCardSerial()) {
        ...
        // First blink LED to indicate card detection
        for (int i = 0; i < 3; i++) {
        ...
        // Keep LED on solid while waiting for card removal
        digitalWrite(LED_BUILTIN, HIGH);
    ...
    // Check if the card has been removed
        ...
        delay(50);  // Reduced debounce delay
    }

    delay(50);  // Reduced main loop delay
}
